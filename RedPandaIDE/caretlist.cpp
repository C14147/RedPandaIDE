...
CaretList::CaretList(QObject* parent) : QObject(parent), mIndex(-1), mPauseAdd(false)
{
}
...
void CaretList::linesDeleted(const Editor* editor, int firstLine, int count)
{
    ...
    for (int i = mList.count() - 1; i >= 0; i--) {
        if (mList[i]->editor == editor && mList[i]->line >= firstLine) {
...
void CaretList::linesInserted(const Editor* editor, int firstLine, int count)
{
    ...
    for (PEditorCaret& caret : mList) {
        if (caret->editor == editor && caret->line >= firstLine)
            caret->line += count;
    }
}
