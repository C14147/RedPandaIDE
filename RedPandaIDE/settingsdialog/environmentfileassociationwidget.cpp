...
EnvironmentFileAssociationWidget::EnvironmentFileAssociationWidget(const QString& name,
                                                                   const QString& group,
                                                                   QWidget* parent)
    : SettingsWidget(name, group, parent), ui(new Ui::EnvironmentFileAssociationWidget)
{
    ui->setupUi(this);
    ...
    mModel.addItem("RedPandaIDE Project File", "dev", 5);
    QItemSelectionModel* m = ui->lstFileTypes->selectionModel();
    ui->lstFileTypes->setModel(&mModel);
    delete m;
    connect(&mModel, &FileAssociationModel::associationChanged, [this]() { setSettingsChanged(); });
}
...
void EnvironmentFileAssociationWidget::doSave()
{
    ...
    mModel.updateAssociationStates();
    pSettings->environment().setOpenFilesInSingleInstance(
        ui->rbOpenInSingleApplication->isChecked());
...
void FileAssociationModel::updateAssociationStates()
{
    ...
    foreach (const PFileAssociationItem& item, mItems) {
        item->selected = checkAssociation("." + item->suffix, "DevCpp." + item->suffix,
                                          //                    item->name,
                                          "Open", pSettings->dirs().executable() + " \"%1\"");
...
void FileAssociationModel::saveAssociations()
{
    ...
    foreach (const PFileAssociationItem& item, mItems) {
        if (item->selected == item->defaultSelected && !item->selected)
            continue;
        bool ok;
        ...
        if (!item->selected) {
            ...
            ok = registerAssociation("." + item->suffix, "DevCpp." + item->suffix);
        }
        if (!ok) {
            QMessageBox::critical(nullptr, tr("Register File Association Error"),
    ...
    foreach (const QString& fileType, fileTypes) {
        ...
        if (fileTypeUsed.contains(fileType)) {
            ...
            ok = registerFileType(fileType, item->name, "Open", pSettings->dirs().executable(),
                                  item->icon);
        } else {
            ok = unregisterFileType(fileType);
        }
        if (!ok) {
            QMessageBox::critical(nullptr, tr("Register File Type Error"),
                                  tr("Don't have privilege to register file types!"));
            return;
        }
    }
...
bool FileAssociationModel::checkAssociation(const QString& extension, const QString& filetype,
                                            const QString& verb, const QString& serverApp)
{
    HKEY key;
    LONG result;
    ...
    return (value2 == filetype) && (value1.compare(serverApp, PATH_SENSITIVITY) == 0);
}
...
bool writeRegistry(HKEY parentKey, const QString& subKey, const QString& value)
{
    ...
    HKEY key;
    LONG result =
        RegCreateKeyExW(parentKey, subKey.toStdWString().c_str(), 0, NULL, REG_OPTION_NON_VOLATILE,
                        KEY_ALL_ACCESS, NULL, &key, &disposition);
    if (result != ERROR_SUCCESS) {
        RegCloseKey(key);
        return false;
    }
    result = RegSetValueExW(key, L"", 0, REG_SZ, (const BYTE*)value.toStdWString().c_str(),
...
bool FileAssociationModel::registerFileType(const QString& filetype, const QString& description,
                                            const QString& verb, const QString& serverApp, int icon)
{
