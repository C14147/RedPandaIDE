...
CompilerAutolinkWidget::CompilerAutolinkWidget(const QString& name, const QString& group,
                                               QWidget* parent)
    : SettingsWidget(name, group, parent), mModel(this), ui(new Ui::CompilerAutolinkWidget)
{
    ui->setupUi(this);
...
void CompilerAutolinkWidget::doSave()
{
    ...
    for (const PAutolink& link : mModel.links()) {
        if (!link->header.isEmpty()) {
            pAutolinkManager->setLink(link->header, link->linkOption, link->execUseUTF8);
        }
    }
    try {
        pAutolinkManager->save();
    } catch (FileError e) {
        QMessageBox::critical(this, tr("Save failed."), e.reason(), QMessageBox::Ok);
    }
}

AutolinkModel::AutolinkModel(CompilerAutolinkWidget* widget, QObject* parent)
    : QAbstractTableModel(parent), mWidget(widget)
{
}
    ...
    if (role == Qt::CheckStateRole && index.column() == 1) {
        ...
        if (index.column() == 0) {
            ...
            if (link->header == s)
                return false;
            if (findLink(s) >= 0) {
                QMessageBox::warning(pMainWindow, tr("Header exists"), tr("Header already exists."),
                                     QMessageBox::Ok);
                return false;
            }
            // we must create a new link, becasue mList may share link pointer with the autolink
            // manger
