...
void ShortcutManager::load()
{
    ...
    if (!file.open(QFile::ReadOnly)) {
        QMessageBox::critical(nullptr, tr("Read shortcut config failed"),
                              tr("Can't open shortcut config file '%1' for read.").arg(filename));
        return;
    }
    ...
    if (error.error != QJsonParseError::NoError) {
        QMessageBox::critical(
            nullptr, tr("Read shortcut config failed"),
            tr("Read shortcut config file '%1' failed:%2").arg(filename, error.errorString()));
        return;
    }
    mShortcuts.clear();
...
void ShortcutManager::save()
{
    ...
    if (!file.open(QFile::WriteOnly | QFile::Truncate)) {
        QMessageBox::critical(nullptr, tr("Save shortcut config failed"),
                              tr("Can't open shortcut config file '%1' for write.").arg(filename));
        return;
    }
    QJsonArray array;
    ...
    if (file.write(doc.toJson()) < 0) {
        QMessageBox::critical(nullptr, tr("Save shortcut config failed"),
                              tr("Write to shortcut config file '%1' failed.").arg(filename));
        return;
    }
}
...
void ShortcutManager::applyTo(QAction* action)
{
    ...
    if (!action->shortcut().isEmpty()) {
        action->setToolTip(action->text() +
                           QString("(%1)").arg(QKeySequence::listToString(action->shortcuts())));
    } else {
        action->setToolTip(action->text());
    }
}
