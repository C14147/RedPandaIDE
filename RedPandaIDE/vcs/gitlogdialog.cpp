...
GitLogDialog::GitLogDialog(const QString& folder, QWidget* parent)
    : QDialog(parent), ui(new Ui::GitLogDialog), mModel(folder)
{
    ui->setupUi(this);
    ...
    connect(ui->tblLogs, &QTableView::customContextMenuRequested, this,
...
PGitCommitInfo GitLogModel::commitInfo(const QModelIndex& index) const
{
    ...
    if (!infoCache->contains(row)) {
        GitManager manager;
        QList<PGitCommitInfo> listCommitInfos = manager.log(mFolder, row, 50);
