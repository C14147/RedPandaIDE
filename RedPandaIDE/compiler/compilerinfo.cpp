...
CompilerInfo::CompilerInfo(const QString& name) : mName(name)
{
}
...
PCompilerOption CompilerInfo::addOption(const QString& key, const QString& name,
                                        const QString section, bool isC, bool isCpp, bool isLinker,
                                        const QString& setting, CompilerOptionType type,
                                        const CompileOptionChoiceList& choices)
{
...
PCompilerOption CompilerInfo::addNumberOption(const QString& key, const QString& name,
                                              const QString section, bool isC, bool isCpp,
                                              bool isLinker, const QString& setting,
...
void CompilerInfo::prepareCompilerOptions()
{
    ...
    //    addOption(CC_CMD_OPT_ANSI, QObject::tr("Support all ANSI standard C programs (-ansi)"),
    //    groupName, true, true, false, "-ansi"); addOption(CC_CMD_OPT_NO_ASM, QObject::tr("Do not
    //    recognize asm,inline or typeof as a keyword (-fno-asm)"), groupName, true, true, false,
    //    "-fno-asm"); addOption(CC_CMD_OPT_TRADITIONAL_CPP, QObject::tr("Imitate traditional C
    //    preprocessors (-traditional-cpp)"), groupName, true, true, false, "-traditional-cpp");
    ...
    addOption(CC_CMD_OPT_OPTIMIZE, QObject::tr("Optimization level (-Ox)"), groupName, true, true,
              false, "-O", CompilerOptionType::Choice, sl);
    ...
    addOption(CC_CMD_OPT_STD, QObject::tr("C++ Language standard (-std)"), groupName, false, true,
              false, "-std=", CompilerOptionType::Choice, sl);
    ...
    addOption(C_CMD_OPT_STD, QObject::tr("C Language standard (-std)"), groupName, true, false,
              false, "-std=", CompilerOptionType::Choice, sl);
    ...
    //    addOption(CC_CMD_OPT_ARCH, QObject::tr("Optimize for the following machine (-march)"),
    //    groupName, true, true, false, "-march=", sl); addOption(CC_CMD_OPT_TUNE,
    //    QObject::tr("Optimize less, while maintaining full compatibility (-tune)"), groupName,
    //    true, true, false, "-mtune=", sl);
    ...
    addOption(CC_CMD_OPT_INSTRUCTION, QObject::tr("Enable use of specific instructions (-mx)"),
              groupName, true, true, false, "-m", CompilerOptionType::Choice, sl);

    // 32bit/64bit
    sl.clear();
    sl.append(
        QPair<QString, QString>(QObject::tr("32-bit pointer, 32-bit instruction (-m32)"), "32"));
    sl.append(
        QPair<QString, QString>(QObject::tr("32-bit pointer, 64-bit instruction (-mx32)"), "x32"));
    sl.append(
        QPair<QString, QString>(QObject::tr("64-bit pointer, 64-bit instruction (-m64)"), "64"));
    addOption(CC_CMD_OPT_POINTER_SIZE, QObject::tr("x86 multilib (-mx)"), groupName, true, true,
              true, "-m", CompilerOptionType::Choice, sl);

    addOption(CC_CMD_OPT_DEBUG_INFO, QObject::tr("Generate debugging information (-g3)"), groupName,
              true, true, false, "-g3");
    addOption(CC_CMD_OPT_PROFILE_INFO, QObject::tr("Generate profiling info for analysis (-pg)"),
              groupName, true, true, true, "-pg");
    addOption(CC_CMD_OPT_SYNTAX_ONLY,
              QObject::tr("Only check the code for syntax errors (-fsyntax-only)"), groupName, true,
              true, false, "-fsyntax-only");
    addOption(
        CC_CMD_OPT_ENABLE_GCC_IMPORT_STD,
        QObject::tr("Enable experimental support for GCC standard library modules (-fmodules-ts)"),
        groupName, false, true, false, "-fmodules-ts");

    // Warnings
    groupName = QObject::tr("Warnings");
    addOption(CC_CMD_OPT_INHIBIT_ALL_WARNING, QObject::tr("Inhibit all warning messages (-w)"),
              groupName, true, true, false, "-w");
    addOption(CC_CMD_OPT_WARNING_ALL, QObject::tr("Show most warnings (-Wall)"), groupName, true,
              true, false, "-Wall");
    addOption(CC_CMD_OPT_WARNING_EXTRA, QObject::tr("Show some more warnings (-Wextra)"), groupName,
              true, true, false, "-Wextra");
    addOption(CC_CMD_OPT_CHECK_ISO_CONFORMANCE,
              QObject::tr("Check ISO C/C++ conformance (-pedantic)"), groupName, true, true, false,
              "-pedantic");
    addOption(CC_CMD_OPT_WARNING_AS_ERROR, QObject::tr("Make all warnings into errors (-Werror)"),
              groupName, true, true, false, "-Werror");
    addOption(CC_CMD_OPT_ABORT_ON_ERROR,
              QObject::tr("Abort compilation on first error (-Wfatal-errors)"), groupName, true,
              true, false, "-Wfatal-errors");
    sl.clear();
    ...
    addOption(CC_CMD_OPT_STACK_PROTECTOR,
              QObject::tr("Check for stack smashing attacks (-fstack-protector)"), groupName, false,
              false, true, "-fstack-", CompilerOptionType::Choice, sl);
    sl.clear();
    ...
    addOption(CC_CMD_OPT_ADDRESS_SANITIZER, QObject::tr("Enable Sanitizer (-fsanitize=)"),
              groupName, true, true, true, "-fsanitize=", CompilerOptionType::Choice, sl);

    // Output
    // groupName = QObject::tr("Output");
    // addOption(CC_CMD_OPT_VERBOSE_ASM, QObject::tr("Put comments in generated assembly code
    // (-fverbose-asm)"), groupName, true, true, false, "-fverbose-asm");
    // addOption(CC_CMD_OPT_ONLY_GEN_ASM_CODE, QObject::tr("Do not assemble, compile and generate
    // the assemble code (-S)"), groupName, true, true, false, "-S");
    // addOption(CC_CMD_OPT_STOP_AFTER_PREPROCESSING, QObject::tr("Do not compile, stop after the
    // preprocessing stage (-E)"), groupName, true, true, false, "-E");

    // Linker
    groupName = QObject::tr("Linker");
    addNumberOption(LINK_CMD_OPT_STACK_SIZE, QObject::tr("PE Stack Size"), groupName, false, false,
                    true, "-Wl,--stack,", "MB", 1024 * 1024, 0, 0, 99999);

    addOption(CC_CMD_OPT_USE_PIPE,
              QObject::tr("Use pipes instead of temporary files during compilation (-pipe)"),
              groupName, true, true, false, "-pipe");
    // addOption(LINK_CMD_OPT_LINK_OBJC, QObject::tr("Link an Objective C program (-lobjc)"),
    // groupName, false, false, true, "-lobjc");
    addOption(LINK_CMD_OPT_NO_LINK_STDLIB,
              QObject::tr("Do not use standard system libraries (-nostdlib)"), groupName, false,
              false, true, "-nostdlib");
    addOption(LINK_CMD_OPT_NO_CONSOLE, QObject::tr("Do not create a console window (-mwindows)"),
              groupName, false, false, true, "-mwindows");
    addOption(LINK_CMD_OPT_STRIP_EXE, QObject::tr("Strip executable (-s)"), groupName, false, false,
              true, "-s");
}
...
PCompilerOption CompilerInfoManager::getCompilerOption(CompilerType compilerType,
...
void SDCCCompilerInfo::prepareCompilerOptions()
{
    ...
    addOption(SDCC_CMD_OPT_PROCESSOR, QObject::tr("Processor (-m)"), groupName, true, false, false,
              "-m", CompilerOptionType::Choice, sl);
    ...
    addOption(SDCC_CMD_OPT_STD, QObject::tr("Language standard (--std)"), groupName, true, false,
              false, "--std-", CompilerOptionType::Choice, sl);
    ...
    addOption(SDCC_OPT_MEMORY_MODEL, QObject::tr("Memory model (--model)"), groupName, true, false,
              false, "--model", CompilerOptionType::Choice, sl);

    addOption(SDCC_OPT_XSTACK, QObject::tr("Use external stack"), groupName, true, false, false,
              "--xstack");
    addOption(SDCC_OPT_XRAM_MOVC, QObject::tr("Use movc instead of movx to read from external ram"),
              groupName, true, false, false, "--xram-movc");
    addOption(SDCC_OPT_ACALL_AJMP, QObject::tr("Replaces lcall/ljmp with acall/ajmp"), groupName,
              true, false, false, "--acall-ajmp");
    addOption(SDCC_OPT_NO_XINIT_OPT, QObject::tr("Don't memcpy initialized xram from code"),
              groupName, true, false, false, "--no-xinit-opt");
    addOption(SDCC_OPT_NOSTARTUP, QObject::tr("Don't generate startup code"), groupName, false,
              false, false, "nostartup");

    groupName = QObject::tr("MCU Specification");

    addOption(SDCC_OPT_IRAM_SIZE, QObject::tr("Internal ram size"), groupName, false, false, true,
              "--iram-size", CompilerOptionType::Input);
    addOption(SDCC_OPT_XRAM_LOC, QObject::tr("External ram start location"), groupName, false,
              false, true, "--xram-loc", CompilerOptionType::Input);
    addOption(SDCC_OPT_XRAM_SIZE, QObject::tr("External ram size"), groupName, false, false, true,
              "--xram-size", CompilerOptionType::Input);
    addOption(SDCC_OPT_STACK_LOC, QObject::tr("Stack pointer initial value"), groupName, false,
              false, true, "--stack-loc", CompilerOptionType::Input);
    addOption(SDCC_OPT_XSTACK_LOC, QObject::tr("External stack start location"), groupName, false,
              false, true, "--xstack-loc", CompilerOptionType::Input);
    addOption(SDCC_OPT_DATA_LOC, QObject::tr("Direct data start location"), groupName, false, false,
              true, "--data-loc", CompilerOptionType::Input);
    addOption(SDCC_OPT_CODE_LOC, QObject::tr("Code segment location"), groupName, false, false,
              true, "--code-loc", CompilerOptionType::Input);
    addOption(SDCC_OPT_CODE_SIZE, QObject::tr("Code segment size"), groupName, false, false, true,
              "--code-size", CompilerOptionType::Input);
}
#endif
