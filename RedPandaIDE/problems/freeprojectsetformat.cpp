...
QList<POJProblem> importFreeProblemSet(const QString& filename)
{
    ...
    while (!xml.atEnd()) {
        ...
        case QXmlStreamReader::TokenType::StartElement:
            ...
            if (xml.name() == QLatin1String("item")) {
            ...
            } else if (currentProblem && (xml.name() == QLatin1String("test_input"))) {
                ...
                currentCase->name =
                    QObject::tr("Problem Case %1").arg(currentProblem->cases.count() + 1);
            } else if (currentProblem && xml.name() == QLatin1String("time_limit")) {
                currentEleName = xml.name().toString();
                foreach (const QXmlStreamAttribute& attr, xml.attributes()) {
            ...
            } else if (currentProblem && xml.name() == QLatin1String("memory_limit")) {
                currentEleName = xml.name().toString();
                foreach (const QXmlStreamAttribute& attr, xml.attributes()) {
                    if (attr.name() == QLatin1String("unit")) {
...
void exportFreeProblemSet(const QList<POJProblem>& problems, const QString& filename)
{
    ...
    // fps
    {
        ...
        writer.writeAttribute("url", "https://github.com/zhblue/freeproblemset/");
        {
            ...
            writer.writeAttribute("name", "RedPandaIDE");
            writer.writeAttribute("url", "https://github.com/C14147/RedPandaIDE/");
