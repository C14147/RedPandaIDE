...
#define BEGIN_STYLE_PIXMAPCACHE(a)                                                                 \
    ...
    bool doPixmapCache =                                                                           \
        (!option->rect.isEmpty()) && ((txType <= QTransform::TxTranslate) ||                       \
...
static QLinearGradient qt_fusion_gradient(const QRect& rect, const QBrush& baseColor,
...
static void dark_fusion_draw_arrow(Qt::ArrowType type, QPainter* painter,
                                   const QStyleOption* option, const QRect& rect,
                                   const QColor& color)
{
    if (rect.isEmpty())
        return;
...
void DarkFusionStyle::drawPrimitive(PrimitiveElement elem, const QStyleOption* option,
...
//     {
//         QPixmap
//         pixmap(QLatin1String(":/qt-project.org/styles/commonstyle/images/fusion_groupbox.png"));
//         int topMargin = 0;
...
//             drawPrimitive(reverse ? PE_IndicatorArrowLeft : PE_IndicatorArrowRight, option,
//             painter, widget);
//         }
//         break;
//     }
    ...
    case PE_FrameTabBarBase:
        if (const QStyleOptionTabBarBase* tbb =
            ...
            case QTabBar::RoundedWest:
                painter->drawLine(tbb->rect.left(), tbb->rect.top(), tbb->rect.left(),
                                  tbb->rect.bottom());
                break;
            case QTabBar::RoundedSouth:
                painter->drawLine(tbb->rect.left(), tbb->rect.bottom(), tbb->rect.right(),
                                  tbb->rect.bottom());
                break;
    ...
    case PE_PanelScrollAreaCorner: {
        ...
        //        if (const QStyleOptionHeader *header = qstyleoption_cast<const QStyleOptionHeader
        //        *>(option)) {
        //            QRect r = header->rect;
        ...
        //                 qt_fusion_draw_arrow(Qt::UpArrow, painter, option, r.translated(offset),
        //                 arrowColor);
        //             } else if (header->sortIndicator & QStyleOptionHeader::SortDown) {
        //                 qt_fusion_draw_arrow(Qt::DownArrow, painter, option,
        //                 r.translated(offset), arrowColor);
        //             }
        // #else
        //             if (header->sortIndicator & QStyleOptionHeader::SortUp) {
        //                 qt_fusion_draw_arrow(Qt::DownArrow, painter, option,
    ...
    case PE_IndicatorToolBarSeparator: {
        ...
        if (option->state & State_Horizontal) {
            ...
            painter->drawLine(rect.bottomLeft().x() + offset + 1, rect.bottomLeft().y() - margin,
                              rect.topLeft().x() + offset + 1, rect.topLeft().y() + margin);
            ...
            painter->setPen(QPen(option->palette.window().color().darker(110)));
            painter->drawLine(rect.topLeft().x() + margin, rect.topLeft().y() + offset + 1,
                              rect.topRight().x() - margin, rect.topRight().y() + offset + 1);
        }
    } break;
    case PE_Frame: {
    ...
    case PE_FrameDockWidget:
        ...
        //        painter->save();
        //        if ((option->state & State_Enabled || option->state & State_On) || !(option->state
        //        & State_AutoRaise)) {
        //            if (widget && widget->inherits("QDockWidgetTitleButton")) {
        //                if (option->state & State_MouseOver)
        //                    proxy()->drawPrimitive(PE_PanelButtonCommand, option, painter,
        //                    widget);
        //            } else {
        //                proxy()->drawPrimitive(PE_PanelButtonCommand, option, painter, widget);
        //            }
    ...
    case PE_IndicatorCheckBox:
        ...
        if (const QStyleOptionButton* checkbox =
            ...
            gradient.setColorAt(0, (state & State_Sunken)
                                       ? pressedColor
                                       : option->palette.base().color().lighter(115));
            gradient.setColorAt(0.15, (state & State_Sunken) ? pressedColor
                                                             : option->palette.base().color());
            gradient.setColorAt(1, (state & State_Sunken) ? pressedColor
                                                          : option->palette.base().color());
            ...
            if (checkbox->state & State_NoChange) {
                ...
                painter->drawRect(rect.adjusted(checkMarkPadding, checkMarkPadding,
                ...
                painter->translate(QStyleHelper::calcDpiScaled(-0.8, dpi),
                                   QStyleHelper::calcDpiScaled(0.5, dpi));
    ...
    case PE_IndicatorRadioButton:
        painter->save();
        {
            QColor pressedColor = mergedColors(option->palette.base().color(),
                                               option->palette.windowText().color(), 85);
            painter->setBrush((state & State_Sunken) ? pressedColor
                                                     : option->palette.base().color());
            painter->setRenderHint(QPainter::Antialiasing, true);
            QPainterPath circle;
        ...
        if (option->state & State_Horizontal) {
            for (int i = -3; i < 2; i += 3) {
                for (int j = -8; j < 10; j += 3) {
                    painter->fillRect(rect.center().x() + i, rect.center().y() + j, 2, 2,
                                      calcLightShade());
                    painter->fillRect(rect.center().x() + i, rect.center().y() + j, 1, 1,
                                      calcDarkShade());
                }
            }
        ...
        if (isFlat && !isDown) {
            if (isDefault) {
                ...
                const QPoint points[4] = {
                    QPoint(r.right() - 1, r.bottom() - 1), QPoint(r.right() - 1, r.top() + 1),
                    QPoint(r.left() + 1, r.bottom() - 1), QPoint(r.left() + 1, r.top() + 1)};
...
QIcon DarkFusionStyle::standardIcon(StandardPixmap standardIcon, const QStyleOption* option,
...
void DarkFusionStyle::drawComplexControl(ComplexControl control, const QStyleOptionComplex* option,
                                         QPainter* painter, const QWidget* widget) const
{
#if QT_CONFIG(slider)
    QColor buttonColor = calcButtonColor(option->palette);
        ...
        if (const QStyleOptionSlider* scrollBar =
            ...
            if (styleObject && proxy()->styleHint(SH_ScrollBar_Transient, option, widget)) {
                ...
                QStyle::State oldState = static_cast<QStyle::State>(
                    qvariant_cast<QStyle::State::Int>(styleObject->property("_q_stylestate")));
                ...
                if (!transient || oldPos != scrollBar->sliderPosition ||
                    ...
                    styleObject->setProperty("_q_stylestate",
            ...
            // Paint groove
            if ((!transient || scrollBar->activeSubControls || wasActive) &&
                scrollBar->subControls & SC_ScrollBarGroove) {
                QLinearGradient gradient(rect.center().x(), rect.top(), rect.center().x(),
                                         rect.bottom());
                if (!horizontal)
                    gradient = QLinearGradient(rect.left(), rect.center().y(), rect.right(),
                                               rect.center().y());
            ...
            if (scrollBar->subControls & SC_ScrollBarSlider) {
                if (transient) {
                    QRect rect =
                        scrollBarSlider.adjusted(horizontal ? 1 : 2, horizontal ? 2 : 1, -1, -1);
                    painter->setPen(Qt::NoPen);
                    ...
                    painter->drawRect(pixmapRect.adjusted(horizontal ? -1 : 0, horizontal ? 0 : -1,
                                                          horizontal ? 0 : 1, horizontal ? 1 : 0));
                    ...
                    //                  painter->setPen(subtleEdge);
                    //                  if (horizontal) {
                    ////                    painter->drawLine(scrollBarSlider.topLeft() + QPoint(-2,
                    /// 0), scrollBarSlider.bottomLeft() + QPoint(2, 0)); /
                    /// painter->drawLine(scrollBarSlider.topRight() + QPoint(-2, 0),
                    /// scrollBarSlider.bottomRight() + QPoint(2, 0));
                    //                  } else {
                    ////                    painter->drawLine(pixmapRect.topLeft() + QPoint(0, -2),
                    /// pixmapRect.bottomLeft() + QPoint(0, -2)); /
                    /// painter->drawLine(pixmapRect.topRight() + QPoint(0, 2),
                    /// pixmapRect.bottomRight() + QPoint(0, 2));
                    //                  }
                }
            }
            ...
            if (!transient && scrollBar->subControls & SC_ScrollBarSubLine) {
                ...
                QRect upRect =
                    scrollBarSubLine.adjusted(horizontal ? 0 : 1, horizontal ? 1 : 0,
                ...
                if (option->state & State_Horizontal)
                    arrowType =
                        option->direction == Qt::LeftToRight ? Qt::LeftArrow : Qt::RightArrow;
            ...
            if (!transient && scrollBar->subControls & SC_ScrollBarAddLine) {
                ...
                if (option->state & State_Horizontal)
                    arrowType =
                        option->direction == Qt::LeftToRight ? Qt::RightArrow : Qt::LeftArrow;
...
int DarkFusionStyle::pixelMetric(PixelMetric metric, const QStyleOption* option,
                                 const QWidget* widget) const
{
    switch (metric) {
...
void DarkFusionStyle::drawControl(ControlElement element, const QStyleOption* option,
                                  QPainter* painter, const QWidget* widget) const
{
    QRect rect = option->rect;
    QColor outline = calcOutline(option->palette);
            ...
            if (menuItem->menuItemType == QStyleOptionMenuItem::Separator) {
                ...
                if (!menuItem->text.isEmpty()) {
                    ...
                    proxy()->drawItemText(painter, menuItem->rect.adjusted(margin, 0, -margin, 0),
                                          Qt::AlignLeft | Qt::AlignVCenter, menuItem->palette,
                ...
                painter->drawLine(menuItem->rect.left() + margin + (reverse ? 0 : w),
                                  menuItem->rect.center().y(),
                                  menuItem->rect.right() - margin - (reverse ? w : 0),
                                  menuItem->rect.center().y());
                painter->restore();
                break;
            }
    ...
    case CE_TabBarTabShape:
        ...
        if (const QStyleOptionTab* tab = qstyleoption_cast<const QStyleOptionTab*>(option)) {
            ...
            if (selected) {
                ...
                //                QColor highlight = option->palette.highlight().color();
                //                if (option->state & State_HasFocus && option->state &
            ...
            if (selected) {
                painter->fillRect(rect.left() + 1, rect.bottom() - 1, rect.width() - 2,
                                  rect.bottom() - 1, tabFrameColor);
                painter->fillRect(QRect(rect.bottomRight() + QPoint(-2, -1), QSize(1, 1)),
                                  calcInnerContrastLine());
