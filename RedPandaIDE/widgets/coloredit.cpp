...
QColor ColorEdit::contrast()
{
    ...
    if (abs(((crBg) & 0xFF) - 0x80) <= TOLERANCE && abs(((crBg >> 8) & 0xFF) - 0x80) <= TOLERANCE &&
...
QSize ColorEdit::sizeHint() const
{
    ...
    return QSize{rect.width() + 10, rect.height() + 6};
}
...
void ColorEdit::paintEvent(QPaintEvent*)
{
    ...
    QRect rect =
        QRect(lineWidth(), lineWidth(), width() - 2 * lineWidth(), height() - 2 * lineWidth());
    if (mColor.isValid()) {
...
{
    QColor c =
        QColorDialog::getColor(mColor, nullptr, tr("Color"),
