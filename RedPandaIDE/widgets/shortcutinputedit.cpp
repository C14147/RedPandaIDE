...
void ShortcutInputEdit::keyPressEvent(QKeyEvent* event)
{
    ...
    if (event->key() == Qt::Key_Delete && event->modifiers() == Qt::NoModifier) {
        ...
        // setText("");
        return;
    } else if (event->modifiers() == Qt::ShiftModifier && !event->text().isEmpty() &&
               event->text().at(0).unicode() > 32 && event->text().at(0).unicode() < 127) {
        // setText("");
        return;
        ...
        if (event->modifiers().testFlag(Qt::ShiftModifier) && !event->text().isEmpty() &&
            event->text().at(0).unicode() > 32 && event->text().at(0).unicode() < 127) {
