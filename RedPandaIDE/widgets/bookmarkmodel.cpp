...
BookmarkModel::BookmarkModel(QObject* parent) : QAbstractTableModel(parent), mIsForProject(false)
{
}
...
PBookmark BookmarkModel::bookmark(const QString& filename, int line, bool forProject)
{
    ...
    foreach (PBookmark bookmark, bookmarks) {
        if (bookmark->filename.compare(filename, PATH_SENSITIVITY) == 0 && bookmark->line == line) {
            return bookmark;
        }
    }
    return PBookmark();
...
void BookmarkModel::renameBookmarkFile(const QString& oldFilename, const QString& newFilename,
                                       bool forProject)
{
    QList<PBookmark> bookmarks;
    if (forProject)
...
void BookmarkModel::save(const QString& filename, const QString& projectFolder)
{
    ...
    for (int i = 0; i < list.count(); i++) {
        ...
        QString filename = forProject ? extractRelativePath(projectFolder, bookmark->filename)
                                      : bookmark->filename;
...
QList<PBookmark> BookmarkModel::load(const QString& filename, qint64 criteriaTimestamp,
                                      ...
                                      bool forProject)
{
    ...
    for (int i = bookmarks.count() - 1; i >= 0; i--) {
        PBookmark bookmark = bookmarks[i];
        if (bookmark->filename == filename && bookmark->line >= startLine) {
...
void BookmarkModel::sort(int column, Qt::SortOrder order)
{
    ...
    case 1:
        if (order == Qt::SortOrder::AscendingOrder) {
            auto sorter = [](PBookmark b1, PBookmark b2) { return b1->filename < b2->filename; };
            std::sort(mBookmarks.begin(), mBookmarks.end(), sorter);
            std::sort(mProjectBookmarks.begin(), mProjectBookmarks.end(), sorter);
        } else {
            auto sorter = [](PBookmark b1, PBookmark b2) { return b2->filename < b1->filename; };
            std::sort(mBookmarks.begin(), mBookmarks.end(), sorter);
            std::sort(mProjectBookmarks.begin(), mProjectBookmarks.end(), sorter);
        }
        break;
    case 2:
        if (order == Qt::SortOrder::AscendingOrder) {
            auto sorter = [](PBookmark b1, PBookmark b2) { return b1->line < b2->line; };
            std::sort(mBookmarks.begin(), mBookmarks.end(), sorter);
            std::sort(mProjectBookmarks.begin(), mProjectBookmarks.end(), sorter);
        } else {
            auto sorter = [](PBookmark b1, PBookmark b2) { return b2->line < b1->line; };
            std::sort(mBookmarks.begin(), mBookmarks.end(), sorter);
            std::sort(mProjectBookmarks.begin(), mProjectBookmarks.end(), sorter);
        }
        break;
