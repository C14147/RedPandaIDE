...
// Custom edit box control. This is necessary because the default behavior of a QLineEdit when it
// gains focus is to select all its text, and if a selection is set before gaining focus, it will be
// overridden by the select-all action upon focusing.
...
// Return the last occurrence index of '.' in the string; if not found, return the length of the
// string.
...
QWidget* FilenameEditDelegate::createEditor(QWidget* parent, const QStyleOptionViewItem& /*option*/,
...
// Set the content, and if the item is a file, set the selection on focus gain to exclude the file
// extension.
void FilenameEditDelegate::setEditorData(QWidget* editor, const QModelIndex& index) const
{
    ...
    // Determine whether the currently edited item is a directory or a file; if it's a directory,
    // there's no need to set a selection.
...
void FilenameEditDelegate::setModelData(QWidget* editor, QAbstractItemModel* model,
                                        const QModelIndex& index) const
{
    FilenameLineEdit* lineEdit = (FilenameLineEdit*)editor;
    if (!lineEdit) {
...
void FilenameEditDelegate::updateEditorGeometry(QWidget* editor, const QStyleOptionViewItem& option,
                                                const QModelIndex& index) const
{
    FilenameLineEdit* lineEdit = (FilenameLineEdit*)editor;
    if (!lineEdit) {
