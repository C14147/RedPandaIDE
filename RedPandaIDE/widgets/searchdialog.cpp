...
SearchDialog::SearchDialog(QWidget* parent)
    : QDialog(parent), ui(new Ui::SearchDialog), mSearchOptions()
{
    ...
    connect(mTabBar, &QTabBar::currentChanged, this, &SearchDialog::onTabBarCurrentChanged);
...
void SearchDialog::doSearch(bool backward)
{
    ...
    if (editor) {
        ...
        int foundCount = editor->searchReplace(
            ui->cbFind->currentText(), "", mSearchOptions, searchEngine, nullptr,
            [this, backward]() {
                QString msg;
                if (backward) {
                ...
                return QMessageBox::question(p, tr("Continue Search"), msg,
        ...
        if (foundCount == 0) {
            ...
            QMessageBox::information(p, tr("Not Found"),
...
void SearchDialog::doReplace(bool replaceAll)
{
    ...
    if (editor) {
        ...
        editor->searchReplace(
            ui->cbFind->currentText(), ui->cbReplace->currentText(), mSearchOptions, searchEngine,
            [&replaceAll](const QString& /*sSearch*/, const QString& /*sReplace*/, int /*Line*/,
                          int /*ch*/, int /*wordLen*/) {
                if (replaceAll) {
