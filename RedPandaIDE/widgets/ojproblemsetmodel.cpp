...
void OJProblemSetModel::addProblems(const QList<POJProblem>& problems)
{
    if (problems.isEmpty())
        return;
    beginInsertRows(QModelIndex(), mProblemSet.problems.count(),
                    mProblemSet.problems.count() + problems.count() - 1);
...
void OJProblemSetModel::loadFromFile(const QString& fileName, int& currentIndex)
{
    ...
    if (file.open(QFile::ReadOnly)) {
        ...
        for (const QJsonValue& problemVal : problemsArray) {
            ...
            problem->memoryLimitUnit =
                (ProblemMemoryLimitUnit)problemObj["memory_limit_unit"].toInt();
...
void OJProblemSetModel::updateProblemAnswerFilename(const QString& oldFilename,
...
bool OJProblemSetModel::moveRows(const QModelIndex& /*sourceParent*/, int sourceRow, int count,
                                 const QModelIndex& /*destinationParent*/, int destinationChild)
{
    if (sourceRow < 0 || sourceRow + count - 1 >= mProblemSet.problems.count() ||
        destinationChild < 0 || destinationChild > mProblemSet.problems.count() ||
        sourceRow == destinationChild || count <= 0) {
        return false;
    }
...
QString OJProblemModel::getTitle()
{
    ...
    QString title = QString("%1 (%2/%3)").arg(mProblem->name).arg(passed).arg(total);
...
bool OJProblemModel::dropMimeData(const QMimeData* data, Qt::DropAction action, int row,
...
bool OJProblemModel::removeRows(int row, int count, const QModelIndex& /*parent*/)
{
    ...
    mMoveTargetRow = -1;
    if (sourceRow < 0 || sourceRow + count - 1 >= mProblem->cases.count() || destinationChild < 0 ||
        destinationChild > mProblem->cases.count() || sourceRow == destinationChild || count <= 0) {
        return false;
    }
    if (!beginMoveRows(QModelIndex(), sourceRow, sourceRow + count - 1, QModelIndex(),
                       destinationChild))
