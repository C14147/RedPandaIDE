...
CodeCompletionPopup::CodeCompletionPopup(QWidget* parent)
    : QWidget(parent), mListView(nullptr), mMutex()
{
...
void CodeCompletionPopup::prepareSearch(const QString& preWord, const QStringList& ownerExpression,
                                        const QString& memberOperator,
                                        const QStringList& memberExpression,
                                        const QString& filename, int line, CodeCompletionType type,
                                        const QSet<QString>& customKeywords)
{
    ...
    case CodeCompletionType::FunctionWithoutDefinition:
        mIncludedFiles = mParser->getIncludedFiles(filename);
        getCompletionForFunctionWithoutDefinition(preWord, ownerExpression, memberOperator,
                                                  memberExpression, filename, line);
        break;
...
bool CodeCompletionPopup::search(const QString& memberPhrase, bool autoHideOnSingleResult)
{
    ...
    if (!mCompletionStatementList.isEmpty()) {
        ...
        // if only one suggestion, and is exactly the symbol to search, hide the frame (the search
        // is over) if only one suggestion and auto hide , don't show the frame
...
PStatement CodeCompletionPopup::selectedStatement()
{
    if (isEnabled()) {
        ...
        if (mListView->currentIndex().isValid() && (index < mCompletionStatementList.count())) {
...
void CodeCompletionPopup::addChildren(const PStatement& scopeStatement, const QString& fileName,
                                      int line, bool onlyTypes)
{
    ...
    if (onlyTypes) {
        if (!scopeStatement) { // Global scope
            for (const PStatement& childStatement : children) {
                ...
                } else if (isIncluded(childStatement->fileName) ||
...
void CodeCompletionPopup::addFunctionWithoutDefinitionChildren(const PStatement& scopeStatement,
...
void CodeCompletionPopup::addStatement(const PStatement& statement, const QString& fileName,
                                       int line)
{
    if (mAddedStatements.contains(statement->command))
        return;
    if (statement->kind == StatementKind::Constructor ||
        statement->kind == StatementKind::Destructor || statement->kind == StatementKind::Block ||
        ...
        return;
    if ((line != -1) && (line < statement->line) && (fileName == statement->fileName))
        return;
...
void CodeCompletionPopup::filterList(const QString& member)
{
    ...
    foreach (const PStatement& statement, mFullCompletionStatementList) {
        ...
        if (hideSymbolsTwoUnderline && statement->command.startsWith("__")) {
            ...
            foreach (const QChar& ch, member) {
                ...
                if (pos == lastPos + 1) {
                    ...
                    PStatementMathPosition matchPosition =
                        std::make_shared<StatementMatchPosition>();
        ...
        if (mIgnoreCase && matched == len) {
            ...
            if (member.length() > 0) {
                statement->firstMatchLength = statement->matchPositions.front()->end -
                                              statement->matchPositions.front()->start;
                statement->matchPosSpan = statement->matchPositions.last()->end -
                                          statement->matchPositions.front()->start;
            } else
    ...
    if (mRecordUsage) {
        ...
        foreach (const PStatement& statement, mCompletionStatementList) {
            if (statement->usageCount == -1) {
                PSymbolUsage usage =
                    pMainWindow->symbolUsageManager()->findUsage(statement->fullName);
                if (usage) {
        ...
        if (mSortByScope) {
            std::sort(mCompletionStatementList.begin(), mCompletionStatementList.end(),
...
void CodeCompletionPopup::getCompletionFor(QStringList ownerExpression,
                                           const QString& memberOperator,
                                           const QStringList& memberExpression,
                                           const QString& fileName, int line,
                                           const QSet<QString>& customKeywords)
{
    ...
    bool isLambdaReturnType = (memberOperator == "->" && ownerExpression.startsWith("[") &&
    ...
    if (!mParser->freeze())
        return;
    {
        auto action = finally([this] { mParser->unFreeze(); });
        ...
        if (memberOperator.isEmpty() || isLambdaReturnType) {
            ...
            while (scopeStatement) {
                ...
                foreach (const QString& namespaceName, scopeStatement->usingList) {
                    PStatementList namespaceStatementsList = mParser->findNamespace(namespaceName);
                ...
                if (scopeStatement->kind == StatementKind::Lambda) {
                    foreach (const QString& phrase, scopeStatement->lambdaCaptures) {
                        ...
                        PStatement statement = mParser->findStatementOf(
                            scopeStatement->fileName, phrase, scopeStatement->line);
                        if (statement)
            ...
            PEvalStatement ownerStatement =
                mParser->evalExpression(fileName, ownerExpression, scope);
            ...
            if ((memberOperator != "::") && (ownerStatement->kind == EvalStatementKind::Variable)) {
                ...
                if (!classTypeStatement)
                    return;
                // It's a iterator
                if (ownerStatement && ownerStatement->typeStatement &&
                    ...
                    if (STLContainers.contains(parentScope->fullName)) {
                        QString typeName = mParser->findFirstTemplateParamOf(
                            fileName, ownerStatement->templateParams, parentScope);
                        //                        qDebug()<<"typeName"<<typeName<<lastResult->baseStatement->type<<lastResult->baseStatement->command;
                        classTypeStatement =
                            mParser->findTypeDefinitionOf(fileName, typeName, parentScope);
                        if (!classTypeStatement)
                            return;
                    ...
                    // is a smart pointer
                    QString typeName = mParser->findFirstTemplateParamOf(
                        fileName, ownerStatement->baseStatement->type, scope);
                    classTypeStatement = mParser->findTypeDefinitionOf(fileName, typeName, scope);
                    if (!classTypeStatement)
                        return;
                } else {
                ...
                if ((classTypeStatement == scopeTypeStatement) ||
                    (ownerStatement->effectiveTypeStatement->command == "this")) {
...
void CodeCompletionPopup::getCompletionForFunctionWithoutDefinition(
    const QString& preWord, QStringList ownerExpression, const QString& memberOperator,
        ...
        if (memberOperator.isEmpty()) {
            ...
            if (scopeStatement) {
                if (scopeStatement->kind == StatementKind::Namespace) {
                    ...
                    if (namespaceStatementsList) {
                        foreach (const PStatement& namespaceStatement, *namespaceStatementsList) {
                            addFunctionWithoutDefinitionChildren(namespaceStatement, fileName,
                                                                 line);
                        }
                    }
                } else {
            ...
            if (ownerStatement->kind == EvalStatementKind::Namespace) {
                ...
                addKeyword("operator");
                addFunctionWithoutDefinitionChildren(ownerStatement->effectiveTypeStatement,
                                                     fileName, line);
            }
        }
    }
        ...
        return;
    {
        ...
        foreach (const QString& name, namespaceNames) {
            ...
            foreach (const PStatement& statement, *namespaces) {
                if (isIncluded(statement->fileName) || isIncluded(statement->definitionFileName)) {
                    addStatement(statement, fileName, line);
                    continue;
                }
            }
...
void CodeCompletionPopup::getCompletionListForTypes(const QString& preWord, const QString& fileName,
                                                    int line)
{
    if (preWord == "typedef") {
        addKeyword("const");
...
void CodeCompletionPopup::setHideSymbolsStartWithTwoUnderline(
    bool newHideSymbolsStartWithTwoUnderline)
{
...
void CodeCompletionPopup::setColors(
...
CodeCompletionPopup::colors() const
{
    return mColors;
}
...
CodeCompletionListModel::CodeCompletionListModel(const StatementList* statements, QObject* parent)
    : QAbstractListModel(parent), mStatements(statements)
{
}
...
void CodeCompletionListItemDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option,
...
QSize CodeCompletionListItemDelegate::sizeHint(const QStyleOptionViewItem& option,
                                               const QModelIndex& index) const
{
...
void CodeCompletionListItemDelegate::setCurrentSelectionColor(
    const QColor& newCurrentSelectionColor)
{
    mCurrentSelectionColor = newCurrentSelectionColor;
}

CodeCompletionListItemDelegate::CodeCompletionListItemDelegate(CodeCompletionListModel* model,
                                                               QWidget* parent)
