...
CPUDialog::CPUDialog(QWidget* parent)
    : QDialog(parent), ui(new Ui::CPUDialog), mInited(false), mSetting(false)
{
    ...
    PColorSchemeItem item =
        pColorManager->getItem(pSettings->editor().colorScheme(), COLOR_SCHEME_ACTIVE_LINE);
    if (item) {
    ...
    onUpdateIcons();
    connect(pIconsManager, &IconsManager::actionIconsUpdated, this, &CPUDialog::onUpdateIcons);
}
...
void CPUDialog::setDisassembly(const QString& file, const QString& funcName,
                               const QStringList& lines, const QList<PTrace>& traces)
{
    mSetting = true;
...
void CPUDialog::updateSyntaxer()
{
    if (pSettings->debugger().blendMode() &&
        pMainWindow->debugger()->supportDisassemlyBlendMode()) {
        if (pSettings->debugger().useIntelStyle())
            ui->txtCode->setSyntaxer(
                syntaxerManager.getSyntaxer(QSynedit::ProgrammingLanguage::MixedAssembly));
        else
            ui->txtCode->setSyntaxer(
                syntaxerManager.getSyntaxer(QSynedit::ProgrammingLanguage::MixedATTAssembly));
    } else {
        if (pSettings->debugger().useIntelStyle())
            ui->txtCode->setSyntaxer(
                syntaxerManager.getSyntaxer(QSynedit::ProgrammingLanguage::Assembly));
        else
            ui->txtCode->setSyntaxer(
                syntaxerManager.getSyntaxer(QSynedit::ProgrammingLanguage::ATTAssembly));
    }
    syntaxerManager.applyColorScheme(ui->txtCode->syntaxer(), pSettings->editor().colorScheme());
}
...
{
    pIconsManager->setIcon(ui->btnStepIntoInstruction,
                           IconsManager::ACTION_RUN_STEP_INTO_INSTRUCTION);
    pIconsManager->setIcon(ui->btnStepOverInstruction,
                           IconsManager::ACTION_RUN_STEP_OVER_INSTRUCTION);
}
