...
CompileArgumentsWidget::CompileArgumentsWidget(QWidget* parent) : QTabWidget(parent)
{
}
        ...
        if (pLayout != nullptr) {
            for (int j = 1; j < pLayout->rowCount() - 1; j++) {
                ...
                PCompilerOption pOption =
                    CompilerInfoManager::getCompilerOption(mCompilerType, key);
                if (!pOption)
                    continue;
                switch (pOption->type) {
                case CompilerOptionType::Checkbox: {
                    QCheckBox* pCheckbox =
                        static_cast<QCheckBox*>(pLayout->itemAtPosition(j, 1)->widget());
                ...
                case CompilerOptionType::Choice: {
                    QComboBox* pCombo =
                        static_cast<QComboBox*>(pLayout->itemAtPosition(j, 2)->widget());
                ...
                case CompilerOptionType::Input: {
                    QLineEdit* pText =
                        static_cast<QLineEdit*>(pLayout->itemAtPosition(j, 2)->widget());
                ...
                case CompilerOptionType::Number: {
                    QSpinBox* pInput =
                        static_cast<QSpinBox*>(pLayout->itemAtPosition(j, 2)->widget());
...
void CompileArgumentsWidget::resetUI(Settings::PCompilerSet pSet,
                                     const QMap<QString, QString>& options)
{
    ...
    for (int i = 0; i < pTab->count(); i++) {
        ...
        QSpacerItem* verticalSpacer =
            new QSpacerItem(10, 100, QSizePolicy::Minimum, QSizePolicy::Expanding);
