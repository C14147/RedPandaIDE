...
SearchInFileDialog::SearchInFileDialog(QWidget* parent)
    : QDialog(parent), ui(new Ui::SearchInFileDialog)
{
...
void SearchInFileDialog::findInFiles(const QString& keyword, SearchFileScope scope,
                                     QSynedit::SearchOptions options, const QString& folder,
...
void SearchInFileDialog::doSearch(bool replace)
{
    ...
    if (ui->rbOpenFiles->isChecked()) {
        PSearchResults results = pMainWindow->searchResultModel()->addSearchResults(
            keyword, mSearchOptions, SearchFileScope::openedFiles);
        ...
        for (int i = 0; i < pMainWindow->editorList()->pageCount(); i++) {
            ...
            if (e != nullptr) {
                fileSearched++;
                PSearchResultTreeItem parentItem = batchFindInEditor(e, e->filename(), keyword);
        ...
        PSearchResults results = pMainWindow->searchResultModel()->addSearchResults(
            keyword, mSearchOptions, SearchFileScope::Folder, ui->txtFolder->text(),
            ui->txtFilters->text(), ui->cbSearchSubFolders);
        if (ui->txtFilters->text().trimmed().isEmpty()) {
            ui->txtFilters->setText("*.*");
        }
        QDir rootDir(ui->txtFolder->text());
        // loop through editors, add results to message control
        QProgressDialog progressDlg(tr("Searching..."), tr("Abort"), 0, 1, pMainWindow);
        ...
        while (!dirs.isEmpty()) {
            ...
            foreach (const QFileInfo& entry,
                     dir.entryInfoList(ui->txtFilters->text().split(";"), filterOptions)) {
                files.append(entry);
            }
        }
        ...
        foreach (const QFileInfo& info, files) {
            ...
            if (e) {
                ...
                try {
                    editor.document()->loadFromFile(curFilename, ENCODING_AUTO_DETECT,
                                                    realEncoding);
                ...
                fileSearched++;
                PSearchResultTreeItem parentItem = batchFindInEditor(&editor, curFilename, keyword);
                int t = parentItem->results.size();
                // findCount+=t;
                if (t > 0) {
                    fileHitted++;
        ...
        PSearchResults results = pMainWindow->searchResultModel()->addSearchResults(
            keyword, mSearchOptions, SearchFileScope::currentFile);
        ...
        if (!pMainWindow->project())
            return;
        PSearchResults results = pMainWindow->searchResultModel()->addSearchResults(
            keyword, mSearchOptions, SearchFileScope::wholeProject);
        QByteArray projectEncoding = pMainWindow->project()->options().encoding;
        QProgressDialog progressDlg(tr("Searching..."), tr("Abort"), 0,
                                    pMainWindow->project()->unitList().count(), pMainWindow);
...
int SearchInFileDialog::execute(QSynedit::QSynEdit* editor, const QString& sSearch,
                                const QString& sReplace, QSynedit::SearchMathedProc matchCallback,
                                QSynedit::SearchConfirmAroundProc confirmAroundCallback)
{
    ...
    if (!mSearchOptions.testFlag(QSynedit::ssoEntireScope) &&
        !mSearchOptions.testFlag(QSynedit::ssoSelectedOnly) && editor->selAvail()) {
    ...
    return editor->searchReplace(sSearch, sReplace, mSearchOptions, searchEngine, matchCallback,
                                 confirmAroundCallback);
}

std::shared_ptr<SearchResultTreeItem> SearchInFileDialog::batchFindInEditor(QSynedit::QSynEdit* e,
                                                                            ...
                                                                            const QString& keyword)
{
    ...
    execute(
        e, keyword, "",
        [e, &parentItem, filename](const QString&, const QString&, int line, int ch, int wordLen) {
    ...
    e->setCaretAndSelection(caretBackup, blockBeginBackup, blockEndBackup);
    return parentItem;
}
...
void SearchInFileDialog::showEvent(QShowEvent* event)
{
    ...
    if (pSettings->environment().language() == "zh_CN") {
        ui->txtRegExpHelp->setText(
            QString("<html><head/><body><p><a href=\"%1\"><span style=\" text-decoration: "
                    "underline; color:#0000ff;\">(?)</span></a></p></body></html>")
                .arg("https://www.runoob.com/regexp/regexp-tutorial.html"));
    } else {
        ui->txtRegExpHelp->setText(
            QString("<html><head/><body><p><a href=\"%1\"><span style=\" text-decoration: "
                    "underline; color:#0000ff;\">(?)</span></a></p></body></html>")
                .arg("https://docs.microsoft.com/en-us/dotnet/standard/base-types/"
...
{
    QString folder =
        QFileDialog::getExistingDirectory(this, tr("Choose Folder"), ui->txtFolder->text());
