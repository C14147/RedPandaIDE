...
bool CppRefacter::findOccurence(Editor* editor, const QSynedit::BufferCoord& pos)
{
    ...
    if (!editor->parser()->freeze())
        return false;
    auto action = finally([&editor] { editor->parser()->unFreeze(); });
    // get full phrase (such as s.name instead of name)
    QStringList expression = editor->getExpressionAtPosition(pos);
    // Find it's definition
    PStatement statement =
        editor->parser()->findStatementOf(editor->filename(), expression, pos.line);
...
bool CppRefacter::findOccurence(Editor* editor, const QString& statementFullname,
...
void CppRefacter::renameSymbol(Editor* editor, const QSynedit::BufferCoord& pos,
                               const QString& newWord)
{
    ...
    if (editor->filename() != oldStatement->fileName ||
        ...
        QMessageBox::critical(editor, tr("Rename Symbol Error"),
    ...
    PStatement newStatement =
        editor->parser()->findStatementOf(editor->filename(), newExpression, pos.line);
    if (newStatement && fullParentName(newStatement) == oldScope) {
        QMessageBox::critical(editor, tr("Rename Symbol Error"), tr("New symbol already exists!"));
        return;
    }
    renameSymbolInFile(editor->filename(), oldStatement, newWord, editor->parser());
}

void CppRefacter::doFindOccurenceInEditor(PStatement statement, Editor* editor,
                                          const PCppParser& parser)
{
    PSearchResults results = pMainWindow->searchResultModel()->addSearchResults(
        statement->command, statement->fullName, SearchFileScope::currentFile);
    PSearchResultTreeItem item =
        findOccurenceInFile(editor->filename(), editor->encodingOption(), statement, parser);
    if (item && !(item->results.isEmpty())) {
        results->results.append(item);
    }
}

void CppRefacter::doFindOccurenceInProject(PStatement statement, std::shared_ptr<Project> project,
                                           const PCppParser& parser)
{
    PSearchResults results = pMainWindow->searchResultModel()->addSearchResults(
        statement->command, statement->fullName, SearchFileScope::wholeProject);
    ...
    foreach (const PProjectUnit& unit, project->unitList()) {
        ...
        if (isCFile(unit->fileName()) || isHFile(unit->fileName())) {
            ...
            if (progressDlg.wasCanceled())
                break;
            PSearchResultTreeItem item = findOccurenceInFile(unit->fileName(),
                                                             unit->encoding() == ENCODING_PROJECT
                                                                 ? project->options().encoding
                                                                 : unit->encoding(),
                                                             statement, parser);
            if (item && !(item->results.isEmpty())) {
                results->results.append(item);
...
PSearchResultTreeItem CppRefacter::findOccurenceInFile(const QString& filename,
                                                       ...
                                                       const PCppParser& parser)
{
    ...
    if (pMainWindow->editorList()->getContentFromOpenedEditor(filename, buffer)) {
        ...
        while (!editor.syntaxer()->eol()) {
            ...
            if (attr && attr->tokenType() == QSynedit::TokenType::Identifier) {
                if (token == statement->command) {
                    ...
                    PStatement tokenStatement =
                        parser->findStatementOf(filename, expression, p.line);
                    if (tokenStatement && (tokenStatement->line == statement->line) &&
...
void CppRefacter::renameSymbolInFile(const QString& filename, const PStatement& statement,
                                     const QString& newWord, const PCppParser& parser)
{
    ...
    if (oldEditor) {
        ...
        try {
            ...
            QMessageBox::critical(pMainWindow, tr("Rename Symbol Error"), e.reason());
            return;
        }
